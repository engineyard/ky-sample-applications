...
# we discrimiate the cases via the variable WORKFLOW_RELEASE_SUMMARY (already existing via deis):
...
then
    ...
    then
        ...
        # No need to block Deployment here. Mark the migration pod ready for the Deis readiness probe
        touch /tmp/migration-ready
    fi
else
    # No need to block Deployment here. Mark the migration pod ready for the Deis readiness probe
